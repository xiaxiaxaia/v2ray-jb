...
generateNginxSpeedtestFile() {
    ...
    if [[ -f "${speedtestFile}" ]]; then
        local currentSize
        currentSize=$(wc -c <"${speedtestFile}" 2>/dev/null || echo 0)
        if [[ "${currentSize}" -ge "${targetSize}" ]]; then
            return 0
        fi
    fi

    rm -f "${speedtestFile}" >/dev/null 2>&1
    if command -v fallocate >/dev/null 2>&1; then
        fallocate -l 200M "${speedtestFile}"
    elif command -v truncate >/dev/null 2>&1; then
        truncate -s 200M "${speedtestFile}"
    else
        dd if=/dev/zero of="${speedtestFile}" bs=1M count=200 status=none
    fi
    ...
    if [[ -d "${nginxStaticPath}" && -f "${nginxStaticPath}/check" ]]; then
        ...
        if [[ "${nginxBlogInstallStatus}" == "y" ]]; then
            ...
            if [[ "${release}" == "alpine" ]]; then
                ...
                wget -q "${wgetShowProgressStatus}" -P "${nginxStaticPath}" "https://raw.githubusercontent.com/panhuanghe/v2ray-agent/master/fodder/blog/unable/html${randomNum}.zip"
            fi
...
updateNginxBlog() {
    ...
    if [[ "${selectInstallNginxBlogType}" =~ ^[1-9]$ ]]; then
        ...
        if [[ "${release}" == "alpine" ]]; then
            ...
            wget -q "${wgetShowProgressStatus}" -P "${nginxStaticPath}" "https://raw.githubusercontent.com/panhuanghe/v2ray-agent/master/fodder/blog/unable/html${selectInstallNginxBlogType}.zip"
        fi
...
updateV2RayAgent() {
    ...
    if [[ "${release}" == "alpine" ]]; then
        ...
        wget -c -q "${wgetShowProgressStatus}" -P /etc/v2ray-agent/ -N --no-check-certificate "https://raw.githubusercontent.com/panhuanghe/v2ray-agent/master/install.sh"
    fi
            ...
            find /etc/v2ray-agent/subscribe_local/default/* | while read -r email; do
                ...
                if [[ -z "${showStatus}" ]]; then
                    ...
                    if [[ -f "/etc/v2ray-agent/subscribe_local/sing-box/${email}" ]]; then
                        ...
                        if [[ "${release}" == "alpine" ]]; then
                            wget -O "/etc/v2ray-agent/subscribe/sing-box/${emailMd5}" -q "https://raw.githubusercontent.com/panhuanghe/v2ray-agent/master/documents/sing-box.json"
                        else
                            wget -O "/etc/v2ray-agent/subscribe/sing-box/${emailMd5}" -q "${wgetShowProgressStatus}" "https://raw.githubusercontent.com/panhuanghe/v2ray-agent/master/documents/sing-box.json"
                        fi
