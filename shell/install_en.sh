...
rebuildAloneConfEn() {
    ...
    if [[ -z "${domain}" || -z "${port}" ]]; then
        echoContent red " ---> Missing domain or port, cannot rebuild alone.conf"
        return
    fi
...
acmeInstallSSL() {
    if [[ "${dnsSSLStatus}" == "true" ]]; then
        ...
        if [[ -n "${txtValue}" ]]; then
            ...
            echoContent yellow " ---> Please refer to this tutorial for adding method, https://github.com/panhuanghe/v2ray-agent/blob/master/documents/dns_txt.md"
